...
NSString *appName = @"KomaruGram.app";
NSString *appDir = nil;
...
void RemoveQuarantineFromBundle(NSString *path) {
	...
	RemoveQuarantineAttribute([path stringByAppendingString:@"/Contents/MacOS/KomaruGram"]);
...
int main(int argc, const char * argv[]) {
	...
	if (update) {
		...
		// Take the Updater (this currently running binary) from the place where it was placed by KomaruGram
		// and copy it to the folder with the new version of the app (ready),
		// so it won't be deleted when we will clear the "KomaruGram.app/Contents" folder.
