...
PreLaunchWindow::PreLaunchWindow(QString title) {
	...
	setWindowTitle(title.isEmpty() ? u"KomaruGram"_q : title);
...
, _log(this)
, _close(this) {
	_label.setText(u"Could not start KomaruGram!\nYou can see complete log below:"_q);
...
, _launch(std::move(launch)) {
	...
	if (_sendingState == SendingNoReport) {
		_label.setText(u"Last time KomaruGram was not closed properly."_q);
	} else {
		_label.setText(u"Last time KomaruGram crashed :("_q);
	}
	...
	_getApp.setText(u"GET THE LATEST VERSION OF KOMARUGRAM"_q);
...
void LastCrashedWindow::updateControls() {
	...
	QSize s(2 * padding + QFontMetrics(_label.font()).horizontalAdvance(u"Last time KomaruGram was not closed properly."_q) + padding + _networkSettings.width(), h);
	if (s == size()) {
		resizeEvent(0);
	} else {
		resize(s);
